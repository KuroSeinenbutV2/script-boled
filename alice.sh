...
ZIPNAME=$(cat "arch/arm64/configs/$DEVICE_DEFCONFIG" | grep "CONFIG_LOCALVERSION=" | sed 's/CONFIG_LOCALVERSION="-*//g' | sed 's/"*//g' )
...
gen_zip() {
	...
	cdir AnyKernel3 || exit
	zip -r [$DATE2][$KERVER][CLANG-15]$ZIPNAME[$VARIANT][$DEVICE][$TYPE][$DATE]$HEADCOMMITID ./* -x .git README.md

	## Prepare a final zip variable
	ZIP_FINAL="[$DATE2][$KERVER][CLANG-15]$ZIPNAME[$VARIANT][$DEVICE][$TYPE][$DATE]$HEADCOMMITID.zip"
